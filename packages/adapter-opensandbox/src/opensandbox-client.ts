...
export class OpenSandboxProtocolClient {
  ...
  async uploadFiles(sandboxId: string, files: readonly { path: string; contentBase64: string }[]): Promise<void> {
  ...
  async getFilesystemDiff(sandboxId: string): Promise<OpenSandboxDiffWireResponse> {
    try {
      return await this.request<OpenSandboxDiffWireResponse>('GET', `/api/v1/sandboxes/${sandboxId}/files/diff`);
    } catch {
  ...
  async executeCommand(
    ...
    const res = await this.request<ExecWireResponse & { stdout?: string | undefined; stderr?: string | undefined }>(
      'POST',
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        throw new SandboxRuntimeError('ERR_PROV_TIMEOUT', `OpenSandbox request timed out after ${this.timeoutMs}ms`, 'opensandbox', true);
      }
      throw new SandboxRuntimeError('ERR_PROV_UNREACHABLE', err.message || 'OpenSandbox unreachable', 'opensandbox', true);
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  private normalizeError(status: number, statusText: string): SandboxRuntimeError {
    switch (status) {
      case 400:
        return new SandboxRuntimeError('ERR_SPEC_INVALID', `Invalid sandbox specification: ${statusText}`, 'opensandbox', false);
      case 404:
        return new SandboxRuntimeError('ERR_PROV_NOT_FOUND', `Resource not found on OpenSandbox: ${statusText}`, 'opensandbox', false);
      case 429:
        return new SandboxRuntimeError('ERR_PROV_QUOTA_EXCEEDED', `Rate limit exceeded: ${statusText}`, 'opensandbox', true);
      case 503:
      case 502:
        return new SandboxRuntimeError('ERR_PROV_UNREACHABLE', `OpenSandbox daemon unavailable: ${statusText}`, 'opensandbox', true);
      default:
        return new SandboxRuntimeError('ERR_INTERNAL_PROVIDER', `OpenSandbox server error (${status}): ${statusText}`, 'opensandbox', false);
    }
  }
}
