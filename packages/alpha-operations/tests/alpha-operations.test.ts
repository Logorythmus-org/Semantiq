...
describe("Sprint 7 alpha operations runtime", () => {
  ...
  it("enforces granular consent before behavioral instrumentation", () => {
    ...
    const invitation = runtime.acceptAlphaInvitation(
      runtime.createAlphaInvitation(cohort.id, "Developer", "developer").id
    );

    expect(invitation.tokenHash).not.toContain(cohort.id);
    expect(() =>
      runtime.recordProductEvent("WorkspaceCreated", "tester:0", cohort.id, "created")
    ).toThrow("explicit research consent");
  ...
  it("supports feedback triage, experiments, release channels, update safety and beta readiness", () => {
    const runtime = new LocalAlphaOperationsRuntime();
    const feedback = runtime.submitContextualFeedback(
      "Question Editor",
      "Question Intelligence",
      "High",
      "Suggestion comparison was unclear."
    );
    const issue = runtime.linkFeedbackToIssue(feedback.id, "S7-QI", "ux-lead");
    const experiment = runtime.createProductExperiment(
      "EXP-S7-002",
      "Do examples help first question creation?",
      "Examples reduce empty-state confusion.",
      ["Curious non-technical user"],
      "A/B moderated observation",
      ["task completion"],
      "80% draft question",
      "50% abandon",
    ...
    expect(
      runtime.completeExperiment(experiment.id, "Inconclusive", "Repeat with larger sample.")
        .decision
    ).toContain("Repeat");
