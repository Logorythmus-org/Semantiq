...
export class CapabilityNegotiator {
  negotiate(
    ...
    const candidates: { manifest: ProviderCapabilitiesManifest; missing: string[]; fallbacks: any[] }[] = [];
    ...
    for (const manifest of manifests) {
      ...
      if (spec.resources.memoryLimitMebibytes > manifest.compute.maxMemoryMb) {
        missing.push(`memory: requested ${spec.resources.memoryLimitMebibytes}MB > max ${manifest.compute.maxMemoryMb}MB`);
      }
