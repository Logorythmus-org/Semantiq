...
export const validateIdentity = (identity: IdentityAggregate): ValidationResult => {
  const issues = [...required("id", identity.id), ...required("profile.displayName", identity.profile.displayName)];
  return { valid: issues.length === 0, issues };
};

export const validateWorkspace = (workspace: WorkspaceAggregate): ValidationResult => {
  const issues = [...required("id", workspace.id), ...required("ownerId", workspace.ownerId), ...required("name", workspace.name)];
  return { valid: issues.length === 0, issues };
};

export const validateKnowledgeObject = (object: KnowledgeObjectAggregate): ValidationResult => {
  const issues = [...required("id", object.id), ...required("workspaceId", object.workspaceId), ...required("title", object.title)];
  return { valid: issues.length === 0, issues };
};
