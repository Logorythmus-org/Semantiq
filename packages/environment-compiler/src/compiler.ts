...
import type {
  BenchmarkTaskDeclaration,
  CompilationResult,
  IEnvironmentCompiler
} from './types.js';

export class EnvironmentCompiler implements IEnvironmentCompiler {
  private readonly profileCatalog: Readonly<Record<string, { readonly name: string; readonly digest: string }>> = {
  ...
  async validateDeclaration(decl: BenchmarkTaskDeclaration): Promise<{ isValid: boolean; errors: readonly string[] }> {
    const errors: string[] = [];
