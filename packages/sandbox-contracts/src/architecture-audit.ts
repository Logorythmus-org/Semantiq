...
export class SandboxArchitectureAuditEngine {
  ...
  formatArchitectureAuditMarkdown(report: SandboxArchitectureAuditReport): string {
    ...
    lines.push('- ✅ **SemantIQ is not a sandbox vendor**: Zero proprietary runtime daemons in Core.');
    lines.push('- ✅ **Clean Protocol Decoupling**: Core defines contracts; providers implement SDK adapters.');
    lines.push('- ✅ **Observable Behavioral Grounding**: Strict 7-stage chain evaluation without claiming access to internal cognition.');
    lines.push('- ✅ **Local-First & Open-Source Viability**: 100% functional offline on local Docker/Podman.');
    lines.push('');
