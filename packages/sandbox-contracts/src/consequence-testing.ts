...
export class ConsequenceTestingEngine {
  evaluateConsequenceTelemetry(
    ...
    for (const spec of specs) {
      ...
      const recognitionEvent = subsequentEvents.find(e =>
        e.stage === 'RESULT' && (e.payload['exitCode'] === 1 || e.payload['passed'] === false)
      );
      const recognized = recognitionEvent !== undefined;
      const recognitionLatencySteps = recognized ? recognitionEvent.seq - manifestedStep : subsequentEvents.length;
      ...
      const attributionEvent = subsequentEvents.find(e =>
      ...
      const remediationEvent = subsequentEvents.find(e =>
      ...
      const secondaryErrors = subsequentEvents.filter(e =>
        e.seq > (recognitionEvent?.seq ?? manifestedStep) &&
        e.stage === 'RESULT' &&
    ...
    const recognitionRate = totalConsequences > 0 ? Number((recognizedCount / totalConsequences).toFixed(4)) : 1.0;
    const attributionAccuracyRate = totalConsequences > 0 ? Number((attributedCount / totalConsequences).toFixed(4)) : 1.0;
    const remediationSuccessRate = totalConsequences > 0 ? Number((remediatedCount / totalConsequences).toFixed(4)) : 1.0;

    const meanRecognitionLatencySteps = recognizedCount > 0
      ? Number((observationEvents.filter(e => e.recognized).reduce((acc, e) => acc + e.recognitionLatencySteps, 0) / recognizedCount).toFixed(2))
      : 10.0;

    const totalSecondaryCascades = observationEvents.reduce((acc, e) => acc + e.secondaryConsequencesCount, 0);
    ...
    const caiRaw =
      0.30 * recognitionRate +
      0.35 * attributionAccuracyRate +
      0.25 * remediationSuccessRate +
      0.10 * (1.0 - Math.min(1.0, meanRecognitionLatencySteps / 10.0)) -
    ...
    } else if (consequenceAwarenessIndex >= 0.40) {
