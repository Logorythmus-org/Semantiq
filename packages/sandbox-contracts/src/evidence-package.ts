...
import type { EvaluatedArtifactEntry, VerifiableBenchmarkExecutionReceipt } from './execution-receipt.js';
...
export class EvidencePackageManager {
  buildPackage(params: {
    ...
    const fileEntries = [
      ...
      ...params.behavioralTrace.map(e => ({ path: `trace/${e.seq}-${e.stage}.json`, sha256: e.payloadDigest }))
    ];
  ...
  validatePackage(pkg: PortableEvidencePackage): EvidencePackageValidationResult {
    ...
    for (const event of pkg.behavioralTrace) {
      if (event.seq <= lastSeq) {
        errors.push(`Sequence continuity broken at event ${event.eventId}: seq ${event.seq} <= previous ${lastSeq}`);
      }
      lastSeq = event.seq;

      // Verify payload digest
      const expectedDigest = computeSha256(canonicalJson(event.payload));
      if (`sha256:${expectedDigest}` !== event.payloadDigest && expectedDigest !== event.payloadDigest) {
    ...
    if (hasRecovery) {
      warnings.push('Behavioral trace contains recovery events: agent encountered and recovered from execution errors.');
    }

    // 2. Validate Merkle Root
    const fileEntries = [
      ...pkg.artifacts.map(a => ({ path: a.path, sha256: a.sha256 })),
      ...pkg.behavioralTrace.map(e => ({ path: `trace/${e.seq}-${e.stage}.json`, sha256: e.payloadDigest }))
    ];
    const computedMerkle = computeMerkleRoot(fileEntries);
    const isMerkleValid = computedMerkle === pkg.packageMerkleRoot;
    if (!isMerkleValid) {
      errors.push(`Package Merkle Root mismatch: expected ${computedMerkle}, received ${pkg.packageMerkleRoot}`);
    }
    ...
    const isBehavioralTraceValid = errors.filter(e => e.includes('event') || e.includes('Sequence')).length === 0;
  ...
  exportPackageSummaryMarkdown(pkg: PortableEvidencePackage): string {
    ...
    for (const e of pkg.behavioralTrace.slice(0, 10)) {
      lines.push(`| ${e.seq} | \`${e.stage}\` | ${e.timestamp} | ${e.actionType ?? 'N/A'} | \`${e.payloadDigest.substring(0, 16)}...\` |`);
    }
    ...
    for (const evalItem of pkg.evaluations) {
      lines.push(`- **${evalItem.metricName}** (${evalItem.evaluatorType}): **${evalItem.score} / ${evalItem.maxScore}** [${evalItem.passed ? 'PASSED' : 'FAILED'}]`);
      lines.push(`  - *Rationale*: ${evalItem.rationale}`);
    }

    lines.push('');
    lines.push('## 3. Financial & Compliance Overview');
    lines.push(`- **Net Billed Cost**: $${pkg.financial.totalNetCostUsd.toFixed(4)} USD (Gross: $${pkg.financial.totalGrossCostUsd.toFixed(4)} USD)`);
