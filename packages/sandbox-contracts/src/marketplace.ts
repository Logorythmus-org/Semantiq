...
export class ProviderMarketplaceEngine {
  ...
  auditListing(listing: ProviderMarketplaceListing): ListingAuditReport {
    ...
    const isTckCertified =
      listing.trustTier === "CRYPTOGRAPHICALLY_CERTIFIED" || listing.trustTier === "TCK_VERIFIED";
  ...
  discover(query: MarketplaceDiscoveryQuery): MarketplaceDiscoveryResult {
    ...
    for (const listing of this.listings.values()) {
      ...
      if (
        query.offlineOnly &&
        (listing.deploymentMode === "MANAGED_MULTI_TENANT" ||
          listing.deploymentMode === "SERVERLESS_MICROVM")
      ) {
        continue;
      }
      ...
      if (
        query.region &&
        listing.privacyProfile.dataStorageRegion !== "local" &&
        listing.privacyProfile.dataStorageRegion !== query.region
      ) {
        continue;
      }
      ...
      // 6. Cost Cap Filter
      if (
        query.maxCostPerUnit !== undefined &&
        listing.costStructure.baseRatePerUnit > query.maxCostPerUnit
      ) {
        continue;
      }
      ...
      if (
        ...
        listing.slaMetrics.p50ColdBootLatencyMs > query.maxColdBootLatencyMs
      ) {
        continue;
      }
      ...
      if (
        ...
        gradeRanks[listing.securityGrade] < gradeRanks[query.minSecurityGrade]
      ) {
        continue;
      }
      ...
      if (query.requiredCapabilities) {
        ...
        if (rc.filesystemDiff && !listing.capabilities.supportsFilesystemDiff)
          capabilityMatch = false;
        if (rc.networkPolicy && !listing.capabilities.supportsNetworkPolicy)
          capabilityMatch = false;
  ...
  private calculateUtilityScore(
    listing: ProviderMarketplaceListing
  ): MarketplaceMatchScoreBreakdown {
    // Cost Score: 1.0 for free local, decaying for higher rates
    const costScore =
      listing.costStructure.billingModel === "FREE_LOCAL"
        ? 1.0
    ...
    const latencyScore = Math.max(
      0.1,
      1.0 - Math.min(1.0, listing.slaMetrics.p50ColdBootLatencyMs / 5000)
    );

    // Isolation Score: MicroVM (1.0), Container (0.7), Restricted Process (0.4)
    const isolationScore =
      listing.securityGrade === "A_HARDENED_MICROVM"
        ? 1.0
    ...
    const trustScore =
      listing.trustTier === "CRYPTOGRAPHICALLY_CERTIFIED"
        ? 1.0
