...
export const providerMarketplaceListingSchema = {
  ...
  properties: {
    ...
    slaMetrics: {
      type: 'object',
      required: ['uptimePercentage', 'p50ColdBootLatencyMs', 'p95ColdBootLatencyMs', 'maxConcurrentSandboxes'],
      properties: {
...
export const complianceAttributionPackageSchema = {
  ...
  properties: {
    ...
    commercialTerms: {
      type: 'object',
      required: ['commercialUseAllowed', 'researchOnlyClause', 'patentRetaliationClause', 'redistributionPermitted'],
      properties: {
...
export const holisticExecutionCostLedgerSchema = {
  ...
  properties: {
    ...
    inference: {
      type: 'object',
      required: ['modelId', 'promptTokens', 'completionTokens', 'reasoningTokens', 'cachedTokens', 'costUsd'],
      properties: {
        modelId: { type: 'string' },
    ...
    runtimeCompute: {
      type: 'object',
      required: ['providerId', 'cpuCoreSeconds', 'ramGibSeconds', 'wallClockDurationMs', 'coldBootSurchargeUsd', 'costUsd'],
      properties: {
        providerId: { type: 'string' },
...
export const verifiableBenchmarkExecutionReceiptSchema = {
  ...
  properties: {
    ...
    provenance: {
      type: 'object',
      required: ['providerId', 'providerVersion', 'runtimeType', 'environmentSpecHash', 'imageDigest', 'isolationMechanism', 'reproducibilityTier'],
      properties: {
        providerId: { type: 'string' },
        ...
        reproducibilityTier: {
          type: 'string',
          enum: ['HERMETIC_DETERMINISTIC', 'PINNED_ENVIRONMENT', 'BEST_EFFORT_REPRODUCIBLE', 'NON_REPRODUCIBLE']
        },
...
export const portableEvidencePackageSchema = {
  ...
  properties: {
    manifest: {
      type: 'object',
      required: ['packageId', 'packageVersion', 'evaluationRunId', 'benchmarkId', 'scenarioId', 'createdAt'],
      properties: {
        packageId: { type: 'string' },
    ...
    evaluations: {
      ...
      items: {
        type: 'object',
        required: ['evaluatorId', 'evaluatorType', 'metricName', 'score', 'maxScore', 'rationale', 'passed'],
        properties: {
...
export const transitionAnalysisReportSchema = {
  ...
  properties: {
    ...
    observedRegimes: {
      ...
      items: {
        type: 'object',
        required: ['regimeName', 'parameterRange', 'characteristicBehavior', 'successRatePercentage'],
        properties: {
    ...
    dataPoints: {
      ...
      items: {
        type: 'object',
        required: ['paramValue', 'trialIndex', 'outcome', 'actionCount', 'recoveryEventsCount', 'recoverySuccessRate', 'loopCycleDetected', 'wallClockDurationMs'],
        properties: {
          paramValue: {},
...
export const failureInjectionReportSchema = {
  ...
  properties: {
    ...
    injectedEvents: {
      ...
      items: {
        type: 'object',
        required: ['faultEventId', 'ruleId', 'stepIndex', 'faultType', 'targetAction', 'injectedOutcome', 'timestamp'],
        properties: {
    ...
    assessments: {
      ...
      items: {
        type: 'object',
        required: ['faultEventId', 'faultType', 'recovered', 'recoveryLatencySteps', 'pathologicalLoopDetected'],
        properties: {
          faultEventId: { type: 'string' },
          faultType: {
            type: 'string',
...
export const longHorizonEvaluationReportSchema = {
  ...
  properties: {
    ...
    milestones: {
      ...
      items: {
        type: 'object',
        required: ['milestoneId', 'phase', 'startStep', 'durationSteps', 'achieved', 'tokensUsed', 'errorsEncountered', 'recoveryCount'],
        properties: {
...
export const sandboxBenchmarkDSLSchema = {
  ...
  properties: {
    ...
    metadata: {
      type: 'object',
      required: ['benchmarkId', 'scenarioId', 'version', 'title', 'description', 'tags', 'license', 'author'],
      properties: {
        benchmarkId: { type: 'string' },
        scenarioId: { type: 'string' },
        version: { type: 'string' },
    ...
    actors: {
      ...
      items: {
        ...
        properties: {
          ...
          permissionLevel: { type: 'string', enum: ['SANDBOX_USER', 'SUDO_ROOT', 'RESTRICTED_READONLY'] }
        }
      }
    },
    tools: {
      ...
      items: {
        ...
        properties: {
          name: { type: 'string' },
          type: { type: 'string', enum: ['BASH', 'FILE_SYSTEM', 'HTTP_API', 'BROWSER', 'MCP_SERVER'] },
          description: { type: 'string' },
          timeoutMs: { type: 'integer', minimum: 100 },
          config: { type: 'object' }
        }
    ...
    lifecycle: {
      type: 'object',
      required: ['setupCommands', 'maxDurationSeconds', 'totalStepBudget', 'retryBudget', 'teardownCommands'],
      properties: {
