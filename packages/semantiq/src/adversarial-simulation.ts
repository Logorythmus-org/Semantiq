  ...
  runScenario(scenarioId: string): SimulationResult {
    switch (scenarioId) {
      ...
      case 'scenario-10-unauthorized-marketing': {
        ...
        const detected = this.humanValidator.detectUnauthorizedEndorsement('Our model is SemantIQ certified this model');
        return {
          scenarioId,
          observedControlBehavior: 'Unauthorized endorsement phrase detected by brand protection scanner',
      ...
      case 'scenario-11-government-misuse': {
        ...
        return {
          scenarioId,
          observedControlBehavior: 'Sole automated decider in high-impact employment domain blocked',
          evidence: report.violations.join('; '),
          status: report.isValid ? 'fail' : 'pass',
          severity: 'critical',
          residualRisk: 'Zero',
      ...
      case 'scenario-12-permanent-emergency-rule': {
        ...
        return {
          scenarioId,
          observedControlBehavior: 'Permanent emergency power rule rejected by constitutional engine',
          evidence: report.violations.join('; '),
          status: report.isValid ? 'fail' : 'pass',
          severity: 'critical',
          residualRisk: 'Zero',
      ...
      case 'scenario-13-dispute-spam': {
        ...
        return {
          scenarioId,
          observedControlBehavior: 'Correction attempt lacking original evidence preservation rejected',
          evidence: report.violations.join('; '),
          status: report.isValid ? 'fail' : 'pass',
          severity: 'medium',
          residualRisk: 'Low',
      ...
      case 'scenario-14-compromised-api-key': {
        ...
        const claimsCheck = this.claimsValidator.scanTextForProhibitedClaims('ghp_123456789012345678901234567890123456');
        return {
          scenarioId,
      ...
      case 'scenario-20-misinterpreted-ranking': {
        ...
        return {
          scenarioId,
          observedControlBehavior: 'Claim lacking mandatory disclaimer and claiming safety rejected',
          evidence: report.violations.join('; '),
          status: report.isValid ? 'fail' : 'pass',
          severity: 'high',
          residualRisk: 'Low',
