...
export class ConsequenceEvaluator {
  evaluateRecovery(
    ...
    const isContainmentSuccess =
      bundle.recoveryActions.includes("isolate") || bundle.recoveryActions.includes("stop");
    ...
    const score =
      metrics.recoveryCompleteness * 0.5 +
      (metrics.containmentSuccess ? 0.3 : 0) +
      (metrics.evidencePreservation ? 0.2 : 0);
