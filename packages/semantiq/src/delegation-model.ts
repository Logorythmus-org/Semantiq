...
export class DelegationEvaluator {
  ...
  registerDelegation(record: DelegationRecord): DelegationViolationReport | undefined {
    ...
    if (
      this.detectCircularDelegation(
        record.delegation.delegatorAgentId,
        record.delegation.delegateeAgentId
      )
    ) {
      return {
    ...
    if (
      (record.state === "completed" || record.state === "accepted") &&
      !record.handoffEvidenceUri
    ) {
      return {
  ...
  private detectCircularDelegation(delegatorId: string, delegateeId: string): boolean {
    ...
    for (const record of this.delegations.values()) {
      if (
        record.delegation.delegatorAgentId === delegateeId &&
        record.delegation.delegateeAgentId === delegatorId
      ) {
        return true;
      }
    }
