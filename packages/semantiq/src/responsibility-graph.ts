...
export class CollectiveResponsibilityGraphEngine {
  ...
  analyzeAccountabilityGaps(): readonly AccountabilityGapReport[] {
    ...
    for (const inc of incidentNodes) {
      const hasRecovery = this.edges.some(
        (e) => e.targetId === inc.id && e.edgeType === "recovered_by"
      );
      if (!hasRecovery) {
        gaps.push({
    ...
    const consequenceNodes = Array.from(this.nodes.values()).filter(
      (n) => n.type === "Consequence"
    );
    for (const cons of consequenceNodes) {
      const contributors = this.edges.filter(
        (e) => e.targetId === cons.id && e.edgeType === "contributed_to"
      );
      if (contributors.length > 1) {
        const uniqueActors = new Set(contributors.map((c) => c.sourceId));
        if (
          uniqueActors.size > 1 &&
          !this.edges.some((e) => e.targetId === cons.id && e.edgeType === "accountable_to")
        ) {
          gaps.push({
