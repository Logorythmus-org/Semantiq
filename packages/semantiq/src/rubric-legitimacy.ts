...
export type EquivalenceStatus = 'validated_equivalent' | 'translated_unvalidated' | 'unknown' | 'conceptually_divergent';
...
export class RubricLegitimacyValidatorEngine {
  ...
  validateMultilingualStatus(record: MultilingualValidationRecord): RubricValidationReport {
    ...
    if (record.equivalenceStatus === 'translated_unvalidated' || record.equivalenceStatus === 'unknown') {
      violations.push(
