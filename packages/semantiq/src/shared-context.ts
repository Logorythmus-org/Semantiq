...
export class SharedMemoryAnalyzer {
  ...
  evaluateReadWrite(event: ReadWriteEvent, allowedWriters: readonly string[] = []): ContextAnomalyReport | undefined {
    const currentVersion = this.latestVersions.get(event.targetKey) ?? 0;

    // 1. Unauthorized Write Check
    if ((event.action === 'write' || event.action === 'patch') && allowedWriters.length > 0 && !allowedWriters.includes(event.agentId)) {
      return {
    ...
    if ((event.action === 'write' || event.action === 'patch') && event.versionAtAction < currentVersion) {
      return {
