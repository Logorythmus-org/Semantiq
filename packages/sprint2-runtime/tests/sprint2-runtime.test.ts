...
import { LocalSprint2Runtime, sprint2ApiContracts, sprint2PromptRegistry, sprint2Screens } from "../src/index.js";
...
describe("Sprint 2 intelligence and research runtime", () => {
  it("runs the critical local question-to-research journey end to end", async () => {
    ...
    expect(result.exportPackage.originalQuestion).toBe("How can AI improve learning in the future?");
  ...
  it("emits the required observable Sprint 2 events", async () => {
    ...
    expect(result.events.every((event) => event.version === 1 && event.actorId && event.workspaceId && event.correlationId && event.audit.localFirst === true)).toBe(true);
  });
  ...
  it("supports rejection audit, Semantiq history, comparison, explanation, search, and contracts", async () => {
    ...
    const rejected = await runtime.rejectSuggestion(result.workspaceId, result.identityId, result.analysis.refinements[0]!.id, "Need a research-ready version instead.");
    const secondReport = await runtime.evaluateQuestion(result.workspaceId, result.identityId, result.question, result.analysis);
    ...
    expect(search.map((item) => item.type)).toEqual(expect.arrayContaining(["research-project", "evidence", "hypothesis", "semantiq-report", "semantic-tag"]));
