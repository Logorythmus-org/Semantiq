...
describe('Sandbox Contracts & Cryptographic Utilities', () => {
  ...
  it('exports valid JSON Schemas', () => {
    ...
    expect(canonicalProviderRegistryEntrySchema.$id).toContain('canonical-provider-registry-entry.json');
    expect(holisticExecutionCostLedgerSchema.$id).toContain('holistic-execution-cost-ledger.json');
    expect(verifiableBenchmarkExecutionReceiptSchema.$id).toContain('verifiable-benchmark-execution-receipt.json');
    ...
    expect(semanticStressEvaluationReportSchema.$id).toContain('semantic-stress-evaluation-report.json');
    ...
    expect(providerConformanceCertificateSchema.$id).toContain('provider-conformance-certificate.json');
    expect(providerCertificationScorecardSchema.$id).toContain('provider-certification-scorecard.json');
    ...
    expect(comprehensiveEvidenceProvenanceGraphSchema.$id).toContain('evidence-provenance-graph.json');
    ...
    expect(sandboxPhaseSecurityAuditReportSchema.$id).toContain('sandbox-phase-security-audit-report.json');
    expect(sandboxEconomicAuditReportSchema.$id).toContain('sandbox-economic-audit-report.json');
    expect(sandboxArchitectureAuditReportSchema.$id).toContain('sandbox-architecture-audit-report.json');
    expect(spisProviderInteroperabilityManifestSchema.$id).toContain('provider-interoperability-manifest.json');
    expect(sandboxPhaseCompletionReportSchema.$id).toContain('sandbox-phase-completion-report.json');
