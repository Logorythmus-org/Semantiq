...
import type { AgentAuthority, AgentCapability, AgentIdentity, AgentRole } from '../../packages/semantiq/src/multi-agent-model.js';
...
describe('Agent Identity, Role, Capability, and Authority (Prompt 9.2)', () => {
  ...
  it('detects capability claim without evidence', () => {
    ...
    const violation = evaluator.evaluateAction(identity, role, authority, baseEvent, unprovenCapability);
    expect(violation).toBeDefined();
