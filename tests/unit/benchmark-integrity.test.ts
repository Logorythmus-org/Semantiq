...
describe('SemantIQ Sandbox Phase — Benchmark Integrity Architecture', () => {
  ...
  it('verifies end-to-end execution integrity and flags rubric tampering', () => {
    const seal = engine.sealManifest(sampleDSL);
    const validReport = engine.verifyExecutionIntegrity(seal, sampleDSL, [event1, event2], 'run-int-001');
    ...
    const tamperedReport = engine.verifyExecutionIntegrity(seal, mutatedDSL, [event1, event2], 'run-int-002');
  ...
  it('formats comprehensive Markdown integrity report', () => {
    const seal = engine.sealManifest(sampleDSL);
    const report = engine.verifyExecutionIntegrity(seal, sampleDSL, [event1, event2], 'run-int-001');
