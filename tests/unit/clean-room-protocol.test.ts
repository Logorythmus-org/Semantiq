...
describe("Clean Room Protocol Verification", () => {
  it("rejects candidate directly inside parent workspace root", () => {
    ...
    expect(result.errors).toContain(
      "Candidate directory must be an isolated workspace outside the parent monorepo root."
    );
  });

  it("rejects candidate inheriting parent .git directory", () => {
    const result = validateCleanRoomProtocol(process.cwd(), {
      isIsolatedCandidate: true,
      hasParentGit: true
    });
    expect(result.valid).toBe(false);
