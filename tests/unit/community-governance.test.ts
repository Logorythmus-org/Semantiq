...
import type { SponsorDisclosureRecord, ConflictDisclosureRecord, CommunityProposalRecord } from '../../packages/semantiq/src/community-governance.js';
...
describe('Phase 11.5.7 — Community Governance and Maintainer Accountability', () => {
  ...
  it('rejects sponsor disclosure with veto power, privileged access, or ranking guarantees', () => {
    ...
    expect(report.violations).toContain('Sponsors are strictly forbidden from holding veto power over evaluation results or governance.');
  });
  ...
  it('requires recusal of conflicted maintainers from voting', () => {
    ...
    expect(report.isValid).toBe(false);
    expect(report.violations[0]).toContain('Maintainer \'maint-01\' has an active conflict of interest');
  });
