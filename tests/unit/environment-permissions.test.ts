...
describe('Environment and Permission Observation Model (Prompt 8.4)', () => {
  ...
  it('redacts GitHub PAT tokens and sensitive secrets from logs', () => {
    const rawLog = 'Connecting with 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 and Bearer secret_jwt_token_123';
    const redacted = redactSecrets(rawLog, ['secret_jwt_token_123']);
    expect(redacted).not.toContain('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');
  ...
  it('detects permission drift between initial manifest and runtime snapshot', () => {
    const manifest: EnvironmentManifest = {
      ...
      version: '1.0.0',
      targetOS: 'windows',
      declaredResources: [{ id: 'res_db', resourceClass: 'database', pathOrEndpoint: 'localhost:5432', isDeclaredInSpec: true }],
      initialGrants: [{ id: 'g_db', resourceId: 'res_db', state: 'read_only', scope: { allowedPathsOrUrls: [] }, grantedAt: '2026-08-01T08:00:00Z', requiresHumanApproval: false }],
      boundary: { id: 'b_001', isSandboxed: true, allowedDomainPatterns: [], rootDirectory: '/app', maxProcessMemoryMb: 512 }
    };

    const snapshot: EnvironmentSnapshot = {
      snapshotId: 'snap_001',
      timestamp: '2026-08-01T09:00:00Z',
      resources: [{ id: 'res_db', resourceClass: 'database', pathOrEndpoint: 'localhost:5432', isDeclaredInSpec: true }],
      grants: [{ id: 'g_db', resourceId: 'res_db', state: 'write', scope: { allowedPathsOrUrls: [] }, grantedAt: '2026-08-01T08:00:00Z', requiresHumanApproval: false }],
