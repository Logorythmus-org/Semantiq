...
import type { ExceptionDecision, ExceptionEvidence, ExceptionRequest, OverrideRecord, ExceptionReview } from '../../packages/semantiq/src/exception-model.js';
...
describe('Exception, Waiver, and Override Records (Prompt 10.16 / 10.4)', () => {
  ...
  const decision: ExceptionDecision = {
    ...
    conditions: [
      { conditionId: 'cond_1', statement: 'Audit log enabled', isFulfilled: true }
    ],
  ...
  it('detects undocumented emergency action failure', () => {
    ...
    const report = engine.evaluateException(request, decision, evidence, undocumentedOverride, review);
    expect(report).toBeDefined();
