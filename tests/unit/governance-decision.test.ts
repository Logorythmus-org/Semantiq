...
describe("Governance Decision Evidence (Prompt 10.5)", () => {
  ...
  const validRecord: GovernanceDecisionRecord = {
    ...
    evidence: [
      {
        evidenceId: "ev_1",
        checksum: { uri: "file:///tmp/ev.json", algorithm: "sha256", hash: "hash1" },
        description: "Pre-flight checks passed"
      }
    ],
    dissents: [
      { dissentId: "dis_1", agentId: "agent_sec", reason: "Concerns regarding load latency" }
    ],
