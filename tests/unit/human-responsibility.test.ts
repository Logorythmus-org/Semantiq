...
import type { HighImpactUseDisclosure, ResponsibilityChainRecord } from '../../packages/semantiq/src/human-responsibility.js';
...
describe('Phase 11.5.3 — Human Responsibility and High-Impact Use', () => {
  ...
  it('rejects responsibility assigned to AI or Benchmark', () => {
    ...
    const report = engine.validateDisclosure(disclosure);
    expect(report.isValid).toBe(false);
    expect(report.violations).toContain('Accountable role cannot be assigned to an AI model or benchmark.');
  });
  ...
  it('detects unauthorized endorsement statements', () => {
    ...
    expect(engine.detectUnauthorizedEndorsement('This report uses SemantIQ benchmarks')).toBe(false);
  });
});
