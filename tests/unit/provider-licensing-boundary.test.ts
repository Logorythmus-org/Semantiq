...
describe('SemantIQ Sandbox Phase — Provider Licensing Boundary', () => {
  ...
  it('verifies network copyleft runtime isolation across gRPC boundary with audit warnings', () => {
    ...
    expect(report.isCompliant).toBe(true);
    expect(report.isCleanRoomIsolated).toBe(true);
    expect(report.warnings.some(w => w.includes('Copyleft runtime (AGPL-3.0-only) detected'))).toBe(true);
    expect(report.violations.length).toBe(0);
  });
  ...
  it('flags non-clean-room implementation and missing attribution notices', () => {
    ...
    expect(report.violations).toContain('Runtime adapter is not declared as a clean-room implementation.');
    expect(report.violations).toContain('Attribution notice is required by license but no third-party notice entries were provided.');
  });
