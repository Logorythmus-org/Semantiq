...
import type { RubricAssumptionManifest, MultilingualValidationRecord, EvaluatorDisagreementRecord } from '../../packages/semantiq/src/rubric-legitimacy.js';
...
describe('Phase 11.5.5 — Rubric Legitimacy and Multilingual Validity', () => {
  ...
  it('rejects a rubric manifest lacking cultural or linguistic assumptions', () => {
    const invalidManifest = { ...validManifest, culturalAssumptions: [], linguisticAssumptions: [] };
    const report = engine.validateRubricManifest(invalidManifest);
    expect(report.isValid).toBe(false);
    expect(report.violations).toContain('Rubric manifest must document explicit cultural assumptions.');
    expect(report.violations).toContain('Rubric manifest must document explicit linguistic assumptions.');
  });
  ...
  it('rejects treating translation alone as validated equivalence', () => {
    ...
    expect(report.isValid).toBe(false);
    expect(report.violations[0]).toContain('Translation alone to \'es\' does not constitute validation.');
  });
