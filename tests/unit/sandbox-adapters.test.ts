...
import { type EnvironmentSpec } from "../../packages/sandbox-contracts/src/index.js";
...
describe("Sandbox Provider Adapters", () => {
  ...
  describe("Cloud Base & E2B Adapter", () => {
    it("manages authentication and scrubs secrets from outputs", () => {
      ...
      const scrubbed = auth.scrubSecrets(
        "Error connecting with e2b_secret_token_12345678901234567890 to backend"
      );
    ...
    it("governs cloud budget and auto-aborts on spend ceilings", () => {
      const governor = new CostQuotaGovernor({
        maxSpendPerRunUsd: 0.05,
