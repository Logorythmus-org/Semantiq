...
describe("Sandbox Evidence Normalization & TCK Conformance", () => {
  ...
  const sampleResult: ExecutionResult = {
    requestId: "req-01",
    exitCode: 0,
    stdout: `\u001b[32m[PASS]\u001b[0m Result with token ${["gh", "p_123456789012345678901234567890123456"].join("")}\r\nDone.`,
    stderr:
  ...
  describe("EvidenceNormalizer", () => {
    it("sanitizes ANSI escapes and redacts secrets across stdout, stderr, and diffs", async () => {
      ...
      const evidence = await normalizer.normalize({
        ...
        delta: sampleDelta,
        agentReasoningTrace:
          "I used sk-123456789012345678901234567890123456789012345678 to query the API.",
      ...
      expect(evidence.result.stdoutSanitized).toContain(
        "[PASS] Result with token [REDACTED_SECRET]"
      );
      expect(evidence.result.stderrSanitized).toContain(
        "[WARN] Minor warning with [REDACTED_SECRET]"
      );
