...
import {
  ScientificClaimsValidatorEngine,
  CANONICAL_DISCLAIMER
} from "../../packages/semantiq/src/scientific-claims.js";
import type {
  ResultClaimRecord,
  ScopeOfClaimBlock
} from "../../packages/semantiq/src/scientific-claims.js";
...
describe("Phase 11.5.2 — Claims Boundary and Scientific Honesty", () => {
  ...
  it("rejects a score missing model version or canonical disclaimer", () => {
    ...
    expect(report.violations).toContain(
      "Result claim must include model ID and explicit model version."
    );
    expect(report.violations).toContain(
      "Result claim is missing the mandatory canonical disclaimer."
    );
  });

  it("scans text for prohibited claims", () => {
    const text =
      "The model understands the prompt and has achieved production ready safety certification.";
