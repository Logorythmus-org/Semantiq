...
import type {
  ...
  CorrectionRecord,
  ContradictionReport
} from "../../packages/semantiq/src/score-disputes.js";
...
describe("Phase 11.5.6 — Score Disputes, Corrections, and Withdrawals", () => {
  ...
  const validDispute: ScoreDisputeRecord = {
    ...
    stateHistory: [
      { state: "submitted", timestamp: "2026-08-07T00:00:00Z", notes: "Initial submission" }
    ]
  };

  it("transitions dispute states preserving complete history", () => {
    const updated = engine.transitionDisputeState(
      validDispute,
      "under_review",
  ...
  it("rejects a correction record that fails to preserve original evidence", () => {
    ...
    expect(report.violations).toContain(
      "Correction records must preserve the original evaluation evidence."
    );
  });
