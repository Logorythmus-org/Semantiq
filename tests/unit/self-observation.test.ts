...
import type { SelfObservationReport, ReplicationRecord } from '../../packages/semantiq/src/self-observation.js';
...
describe('Phase 11.5.8 — SemantIQ Self-Observation', () => {
  ...
  const validReport: SelfObservationReport = {
    ...
    limitations: ['Evaluation currently focused on English text prompts', 'Postgres integration optional']
  };
  ...
  it('rejects a self-observation report missing unknown dimensions or limitations', () => {
    ...
    expect(report.violations).toContain('Self-observation reports must explicitly list unknown dimensions where evidence is unavailable.');
    expect(report.violations).toContain('Self-observation reports must document project limitations.');
  });
  ...
  it('rejects a failed replication record without discrepancy notes', () => {
    ...
    expect(report.isValid).toBe(false);
    expect(report.violations[0]).toContain('Failed reproduction records require explicit discrepancy notes.');
  });
});
