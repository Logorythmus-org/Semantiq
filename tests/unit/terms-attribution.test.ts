...
describe('SemantIQ Sandbox Phase — Terms, Attribution, and Compliance', () => {
  ...
  it('assigns NON_COMMERCIAL_RESTRICTED grade when research-only clause is present', () => {
    ...
    expect(report.warnings.some(w => w.includes('restricted to academic/non-commercial research'))).toBe(true);
  });
    ...
    const customTrademarks: TrademarkDisclaimer[] = [
      {
        ...
        disclaimerText: 'E2B is a trademark of E2B Inc. Used nominatively to refer to execution backend.'
      }
    ];
